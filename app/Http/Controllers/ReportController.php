...
{
    private $product, $material, $type, $customer, $car_brand,
        $car_type, $motor_brand, $motor_type, $warehouse, $suppliers, $all_warehouse,
        $group, $trade_product;
        ...
        if ($request->ajax()) {
            if (!empty($request->from_date)) {
                $invoice = SalesOrderDetailModel::with('productSales', 'salesorders')
                    ...
                    ->whereHas(
                        'salesorders',
                        function ($q) use ($request, $userWarehouseIds) {
                ...
                $invoice = SalesOrderDetailModel::with('soBy', 'productSales', 'salesorders')
                    ->whereHas(
                        'salesorders',
                        function ($q) use($userWarehouseIds) {
                            $q->where('isapprove', 'approve');
                            $q->where('isverified', 1);
                            $q->where('isrejected',0);
            ...
            return datatables()->of($invoice)
                ...
                ->editColumn('paid_date', function ($data) {
                    if ($data->salesorders->paid_date == null) {
                        return "-";
                    } else return date('d M Y', strtotime($data->salesorders->paid_date));
                })
        ...
        if ($request->ajax()) {
            $data = ReturnTradePurchaseDetailModel::with('returnBy', 'productBy')
                ->whereHas('returnBy', function ($q) use ($request) {
                    $q->when($request->from_date, function ($query, $fromDate) use ($request) {
    ...
    public function reportSecondStock(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($request->product) || !empty($request->warehouse)) {
                ...
                $invoice = SecondProductModel::with(['warehouseStockBy'], ['productTradeBy'])->latest()->get();
            }
            return datatables()->of($invoice)
        ...
        $warehouse__ = WarehouseModel::with('typeBy')->whereHas('typeBy', function ($query) {
            $query->where('type', '7');
        })->latest()->get();
        $data = [
   ...
   // ! function report retail
    public function report_retail(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($request->from_date)) {
                ...
                $invoice = DirectSalesDetailModel::with(
                    'productBy',
                    'directSalesBy',
                    ...
                    ->whereHas('directSalesBy', function ($query) use ($request) {
                        ...
                        $query->when($request->from_date, function ($order_date) use ($request) {
                            $order_date->whereBetween('order_date', array($request->from_date, $request->to_date));
                        });
            ...
            return datatables()->of($invoice)
                ...
                ->editColumn('cust_name', function (DirectSalesDetailModel $directSalesDetailModel) {
                    ...
                    } else return $directSalesDetailModel->directSalesBy->cust_name;
                })
                // ! get customer phone
                ->editColumn('cust_phone', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->cust_phone == null) {
                        return "-";
                    } else return $directSalesDetailModel->directSalesBy->cust_phone;
                })
                // ! get customer ktp
                ->editColumn('cust_ktp', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->cust_ktp == null) {
                        return "-";
                    } else return $directSalesDetailModel->directSalesBy->cust_ktp;
                })
                // ! get customer email
                ->editColumn('cust_email', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->cust_email == null) {
                        return "-";
                    } else return $directSalesDetailModel->directSalesBy->cust_email;
                })
                 ->editColumn('area', function (DirectSalesDetailModel $directSalesDetailModel) {
                    ...
                    } else return $directSalesDetailModel->directSalesBy->warehouseBy->areaBy->area_name;
                })
                ...
                ->editColumn('plate_number', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->plate_number == null) {
                        return "-";
                    } else return $directSalesDetailModel->directSalesBy->plate_number;
                })
                // ! get car brand
                ->editColumn('car_brand_id', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->car_brand_id == null) {
                        return '-';
                    } else if (is_numeric($directSalesDetailModel->directSalesBy->car_brand_id)) {
                ...
                ->editColumn('car_type_id', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->car_type_id == null) {
                        return '-';
                    } else if (is_numeric($directSalesDetailModel->directSalesBy->car_type_id)) {
                ...
                ->editColumn('motor_brand_id', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->motor_brand_id == null) {
                        return '-';
                    } else if (is_numeric($directSalesDetailModel->directSalesBy->motor_brand_id)) {
                ...
                ->editColumn('motor_type_id', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->motor_type_id == null) {
                        return '-';
                    } else if (is_numeric($directSalesDetailModel->directSalesBy->motor_type_id)) {
                ...
                ->editColumn('other', function (DirectSalesDetailModel $directSalesDetailModel) {
                    if ($directSalesDetailModel->directSalesBy->other == null) {
                        return "-";
                    } else return $directSalesDetailModel->directSalesBy->other;
                })
                // ! get remark
    ...
    public function report_po(Request $request)
    {
        if ($request->ajax()) {
            if (!empty($request->from_date)) {
                ...
                $purchase = PurchaseOrderDetailModel::with(
                    'purchaseOrderBy',
                    ...
                    ->whereHas('purchaseOrderBy', function ($query) use ($request) {
                        $query->when(
                            $request->from_date,
                            function ($order_date) use ($request) {
    ...
    public function report_receivable(Request $request)
    {
        ...
        $retail = DirectSalesModel::select('*', DB::raw("cust_name AS customers_id"))
            ...
            ->get();
        if ($request->ajax()) {
            $all = $invoice->concat($retail)
                ->when($request->from_date, function ($query) use ($request) {
                    return $query->whereBetween('order_date', array($request->from_date, $request->to_date));
                })
                ...
                ->editColumn('due_date', function ($data) {
                    if ($data instanceof SalesOrderModel) {
                        ...
                        if ($data->due_date == null) {
                            return date('d-m-Y', strtotime($data->order_date));
                        } else return date('d-m-Y', strtotime($data->due_date));
                    }
                })
     ...
     public function report_settlement_direct(Request $request)
    {
        ...
        $invoice = DirectSalesCreditModel::with(
            'directSalesBy',
            'directSalesBy.customerBy',
            'directSalesBy.directSalesDetailBy',
            'directSalesBy.directSalesCreditBy',
            'directSalesBy.createdBy',
            'directSalesBy.directSalesReturnBy'
        )
        ...
        if ($request->ajax()) {
            return datatables()->of($invoice)
                ...
                ->editColumn('payment_recipient', function ($data) {
                    $journal_detail = JournalDetail::where('journal_id', $data->journal_id)->where('status',1)->where('debit', '>', 0)->first();
     ...
     public function report_settlement(Request $request)
    {
        ...
        $invoice = SalesOrderCreditModel::with(
            'salesorders',
            'salesorders.customerBy',
            'salesorders.salesOrderDetailsBy',
            'salesorders.salesOrderCreditsBy',
            'salesorders.createdSalesOrder',
            'salesorders.returnBy'
        )
        ...
        if ($request->ajax()) {
            return datatables()->of($invoice)
                ...
                ->editColumn('payment_recipient', function ($data) {
                    $journal_detail = JournalDetail::where('journal_id', $data->journal_id)->where('debit', '>', 0)->where('status', 1)->first();
                    return $journal_detail?->coa?->name;
                })
                ->editColumn('settlement_period', function ($data) {
     ...
     public function report_settlement_purchase(Request $request)
    {
        $currentMonth = Carbon::now()->month;
        $currentYear = Carbon::now()->year;
        $invoice = PurchaseOrderCreditModel::with(
            'purchaseorders',
            'purchaseorders.supplierBy',
            'purchaseorders.purchaseOrderDetailsBy',
            'purchaseorders.purchaseOrderCreditsBy',
            'purchaseorders.createdPurchaseOrder',
            'purchaseorders.purchaseOrderReturnBy'
        )
            ->whereHas('purchaseorders', function ($query) use ($request, $currentMonth, $currentYear) {
                ...
                // });
                $query->when($request->from_date, function ($query) use ($request) {
                    return $query->whereBetween('paid_date', array($request->from_date, $request->to_date));
                }, function ($query) use ($currentMonth, $currentYear) {
    ...
    {
        $invoice = SalesOrderModel::where('isPaid', 0)
            ->where('paid_date', null)
            ->where('isrejected',0)
            ->select('customers_id')
            ->get();
        ...
        $customer_has_credit = CustomerModel::
            ...
            ->oldest('name_cust')->get();
        if ($request->ajax()) {
            return datatables()->of($customer_has_credit)
        ...
        if ($request->ajax()) {
            if (!empty($request->from_date)) {
                ...
                $invoice = AccuClaimModel::with('productSales', 'carBrandBy', 'carTypeBy', 'loanBy', 'createdBy', 'accuClaimDetailsBy')
                    ->when($request->from_date, function ($query) use ($request) {
                        $query->whereBetween('claim_date', array($request->from_date, $request->to_date));
                    })
            ...
            return datatables()->of($invoice)
                ...
                ->editColumn('customer_id', function (AccuClaimModel $AccuClaimModel) {
                    ...
                    else return $AccuClaimModel->customer_id;
                })
    ...
    public function report_mutation(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($request->from_date)) {
                ...
                $mutation = StockMutationDetailModel::with(
                    'stockMutationBy',
                    'productBy',
                    'stockMutationBy.fromWarehouse',
                    'stockMutationBy.toWarehouse',
                    'productBy.materials',
                    'productBy.sub_materials',
                    'productBy.sub_types',
                )
                    ->whereHas('stockMutationBy', function ($query) use ($request) {
                        $query->where('isapprove', 1);
                        $query->when($request->from_date, function ($query) use ($request) {
                            $query->whereBetween('mutation_date', array($request->from_date, $request->to_date));
                        });
                ...
                $invoice = SecondSaleDetailModel::with('secondProduct', 'second_sale')
                    ->whereHas('second_sale', function ($query) use ($request) {
                        $query->when(
                            $request->from_date,
                            function ($query) use ($request) {
                                $query->whereBetween('second_sale_date', array($request->from_date, $request->to_date));
                            }
                        );
                    })
...
{
    if ($request->ajax()) {
        $query = AttendancesModel::with('userBy');
       ...
       $attendances = $query->latest()->get()->groupBy('user_id');

return datatables()
    ->of($attendances)
    ->editColumn('user_id', function ($group) {
        // Ambil satu data dari grup (misalnya yang pertama) untuk mendapatkan user_id
        $firstItem = $group->first();
        return $firstItem->userBy ? $firstItem->userBy->name : null;
    })
    ->editColumn('date', function ($group) {
        // Ambil satu data dari grup (misalnya yang pertama) untuk mendapatkan tanggal
        $firstItem = $group->first();
        return Carbon::parse($firstItem->clock_time)->format('d M Y');
    })
    ->editColumn('time_in', function ($group) {
        // Ambil satu data dari grup (misalnya yang pertama) untuk mencari time_in
        $firstItem = $group->first();
        $timeIn = AttendancesModel::where('user_id', $firstItem->user_id)
            ->whereDate('clock_time', Carbon::parse($firstItem->clock_time)->format('Y-m-d'))
            ->where('type', 'in')
            ->orderBy('clock_time', 'asc')
            ->first();
        return $timeIn ? Carbon::parse($timeIn->clock_time)->format('H:i:s') : 'No Time In';
    })
    ->editColumn('time_out', function ($group) {
        // Ambil satu data dari grup (misalnya yang pertama) untuk mencari time_out
        $firstItem = $group->first();
        $timeOut = AttendancesModel::where('user_id', $firstItem->user_id)
            ->whereDate('clock_time', Carbon::parse($firstItem->clock_time)->format('Y-m-d'))
            ->where('type', 'out')
            ->orderBy('clock_time', 'desc')
            ->first();
        return $timeOut ? Carbon::parse($timeOut->clock_time)->format('H:i:s') : 'No Time Out';
    })
    ->addIndexColumn()
    ->make(true);
    ...
    {
        if ($request->ajax()) {
            $data_car = CarTypeModel::with('brandBy')->whereHas('brandBy', function ($query) {
                $query->orderBy('car_brand', 'ASC');
            })->get();

            return datatables()->of($data_car)
    ...
    {
        if ($request->ajax()) {
            $data_moto = MotorTypeModel::with('brandBy')->whereHas('brandBy', function ($query) {
                $query->orderBy('name_brand', 'ASC');
            })->get();

            return datatables()->of($data_moto)
    ...
    public function reportStock(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($request->from_date)) {
                ...
                $stock_purchase_order = PurchaseOrderDetailModel::with(
                    'purchaseOrderBy',
                    'purchaseOrderBy.supplierBy',
                    'purchaseOrderBy.createdPurchaseOrder',
                    'purchaseOrderBy.warehouseBy',
                ...
                $stock1 = $stock->transform(function ($item) use (
                    $stock_sales_order,
                    $stock_return_sales_order,
                    $stock_purchase_order,
                    $stock_return_purchase_order,
                    $stock_sales_order_retail,
                    $stock_return_sales_order_retail,
                    $stock_mutation
                ) {
            ...
            return datatables()->of($stock)
                ->editColumn('nama_barang', function ($data) use ($request) {
                    return '<a href="#" class="fw-bold text-success modal-trace" data-product="' . $data->products_id . '"
                    data-warehouse="' . $data->warehouses_id . '" data-stock="' . $data->id . '" data-bs-toggle="modal" data-original-title="test" data-bs-target="#trace' . $data->id . '">' . $data->productBy->sub_materials->nama_sub_material . ' '
                        . $data->productBy->sub_types->type_name . ' ' . $data->productBy->nama_barang . '</a>';
                })
    ...
    public function report_promotion_stock(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($request->from_date)) {
                ...
                // ! get data qty purchase
                $stock_purchase = ItemPromotionPurchaseDetailModel::with(
                    'purchaseBy',
                    'itemBy'
                )
                ...
                $stock1 = $stock->transform(function ($item) use (
                    $stock_transaction,
                    $stock_return_transaction,
                    $stock_purchase,
                    $stock_return_purchase,
                    $stock_mutation
                ) {
                ...
                $stock = DB::table('item_promotion_stocks')
                    ->join('warehouses', 'item_promotion_stocks.id_warehouse', '=', 'warehouses.id')
                    ->join('item_promotions', 'item_promotion_stocks.id_item', '=', 'item_promotions.id')
                    ->select('item_promotion_stocks.*', 'item_promotion_stocks.id AS id_stock', 'item_promotions.*', 'warehouses.*')
                    ->orderBy('item_promotions.name')
            ...
            return datatables()->of($stock)
                ->editColumn('nama_barang', function ($data) use ($request) {
                    return '<a href="#" class="fw-bold text-success modal-trace" data-product="' . $data->id_item . '"
                    data-warehouse="' . $data->id_warehouse . '" data-stock="' . $data->qty . '" data-bs-toggle="modal" data-original-title="test" data-bs-target="#trace' . $data->id_stock . '">'
                        .  $data->name . '</a>';
                })
                ->editColumn('warehouse', function ($data) use ($request) {
    ...
    public function report_promotion_transaction(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($request->from_date)) {
                ...
                $invoice = ItemPromotionTransactionDetailModel::with(
                    'transactionBy',
                    'itemBy',
                    'transactionBy.createdBy',
                    'transactionBy.warehouseBy',
                    'transactionBy.customerBy',
                    'transactionBy.transactionDetailBy'
                )
    ...
    public function report_promotion_purchase(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($request->from_date)) {
                ...
                $invoice = ItemPromotionPurchaseDetailModel::with(
                    'purchaseBy',
                    'itemBy',
                    'purchaseBy.createdBy',
                    'purchaseBy.warehouseBy',
                    'purchaseBy.customerBy',
                    'purchaseBy.purchaseDetailBy'
                )
