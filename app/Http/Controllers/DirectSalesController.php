...
class DirectSalesController extends Controller
{
            ...
            return Datatables::eloquent($direct)
                ...
                ->editColumn('car_brand_id', function (DirectSalesModel $directSalesModel) {
                    if ($directSalesModel->car_brand_id == null) {
                        ...
                        return $directSalesModel->car_brand_id;
                    }
                })
                ->editColumn('car_type_id', function (DirectSalesModel $directSalesModel) {
                    if ($directSalesModel->car_type_id == null) {
                        ...
                        return $directSalesModel->car_type_id;
                    }
                })
                ...
                ->editColumn('motor_brand_id', function (DirectSalesModel $directSalesModel) {
                    if ($directSalesModel->motor_brand_id == null) {
                        ...
                        return $directSalesModel->motor_brand_id;
                    }
                })
                ->editColumn('motor_type_id', function (DirectSalesModel $directSalesModel) {
                    if ($directSalesModel->motor_type_id == null) {
                        ...
                        return $directSalesModel->motor_type_id;
                    }
                })
                ->editColumn('cust_name', function (DirectSalesModel $directSalesModel) {
                    if (is_numeric($directSalesModel->cust_name)) {
                        if ($directSalesModel->customerBy == null) {
                            ...
                            return $directSalesModel->customerBy->code_cust . ' - ' . $directSalesModel->customerBy->name_cust;
                        }
                    } else {
                        return $directSalesModel->cust_name;
                    }
                ...
                ->addColumn('action', function ($direct) {
                    ...
                    return view('direct_sales._option', compact('direct', 'car_brands', 'motor_brands', 'customers'))->render();
                    // return 'direct';
                })
    ...
    public function approval()
    {
        ...
        $dataInvoice = DirectSalesModel::where('isapproved', 0)
            ->where('isrejected', 0)
            ->latest('created_at') //->whereIn('warehouse_id', Auth::user()->userWarehouseBy->pluck('warehouse_id'))
            ->get();
    ...
    public function approve($id)
    {
        try {
            ...
            $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Penjualan Direct No.' . $selected_so->order_number, $selected_so->warehouse_id);
            ...
            if ($journal != '' && $journal != null && $journal != false) {
                // akun piutang
                createJournalDetail($journal, '1-200', $selected_so->order_number, $selected_so->total_incl, 0);

                // akun penjualan

                createJournalDetail($journal, '4-100', $selected_so->order_number, 0, $selected_so->total_excl);

                // akun pajak masukan

                createJournalDetail($journal, '2-300', $selected_so->order_number, 0, $selected_so->total_ppn);
            }

            //Save HPP

            $hpp = createJournal(Carbon::now()->format('Y-m-d'), 'HPP Direct No.' . $selected_so->order_number, $selected_so->warehouse_id);
            ...
            if ($hpp != '' && $hpp != null && $hpp != false) {
                ...
                // akun persediaan
                createJournalDetail($hpp, '1-401', $selected_so->order_number, 0, $hpp_excl + $hpp_ppn);
    ...
    public function rejected(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $invoice = DirectSalesModel::with('createdBy', 'carBrandBy', 'carTypeBy', 'motorBrandBy', 'motorTypeBy', 'directSalesDetailBy', 'directSalesDetailBy.productBy', 'directSalesDetailBy.retailPriceBy')
                ->where('isrejected', 1)
    ...
    public function create()
    {
        ...
        $retail_products = ProductCostModel::with(['productBy', 'warehouseBy', 'productBy.sub_materials', 'productBy.stockBy', 'productBy.sub_types', 'productBy.uoms'])
            ...
            ->get()
            ->sortBy([['productBy.sub_materials.nama_sub_material', 'asc'], ['productBy.sub_types.type_name', 'asc'], ['productBy.nama_barang', 'asc']]);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $kode_area = WarehouseModel::join('customer_areas', 'customer_areas.id', '=', 'warehouses.id_area')
                ...
                ->first();
            $lastRecord = DirectSalesModel::where('warehouse_id', $request->warehouse_id)
                ->latest()
                ->first();
            ...
            if ($request->cust_name == 'other_cust') {
                ...
                } elseif ($getWarehouse->id_area == 10) {
                ...
                if ($request->vehicle == 'Car') {
                    ...
                    $model->other = null;
                } elseif ($request->vehicle == 'Motocycle') {
            ...
            $model->total_excl = $request->total_incl;
            ...
            foreach ($request->retails as $item) {
                ...
                if ($model->isapproved == 1) {
                    $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Penjualan Direct No.' . $model->order_number, $model->warehouse_id);

                    if ($journal != '' && $journal != null && $journal != false) {
                        // akun piutang
                        createJournalDetail($journal, '1-200', $model->order_number, $model->total_incl, 0);

                        // akun penjualan
                        createJournalDetail($journal, '4-100', $model->order_number, 0, $model->total_incl);

                        // akun pajak masukan
                    }

                    //Save HPP
                    $hpp = createJournal(Carbon::now()->format('Y-m-d'), 'HPP Direct No.' . $model->order_number, $model->warehouse_id);
    ...
    public function select()
    {
        ...
        if (request()->has('q')) {
            $search = request()->q;
            $sub_materials = DistrictModel::where('district_name', 'LIKE', "%$search%")->get();
        } else {
    ...
    public function print_invoice($id)
    {
        ...
        $pdf = Pdf::loadView('direct_sales.print_invoice', compact('warehouse', 'data'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->order_number . '.pdf');
    ...
    public function PrintStruk($id)
    {
        ...
        $pdf = Pdf::loadView('direct_sales.print_struk', compact('warehouse', 'data'))->save('pdf/' . $data->order_number . '.pdf');
    ...
    public function print_do($id)
    {
        ...
        $data->save();
        $warehouse = WarehouseModel::where('id', $data->warehouse_id)->first();
        $pdf = Pdf::loadView('direct_sales.delivery_order', compact('warehouse', 'data'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $so_number . '.pdf');
    ...
    public function update_retail(Request $request, $id)
    {
        try {
            ...
            foreach ($request->retails as $check) {
                ...
                $getstock = StockModel::where('products_id', $check['product_id'])
            ...
            foreach ($request->retails as $product) {
                ...
                if ($product_exist != null) {
                    ...
                    $getPrice = ProductCostModel::where('id_product', $product['product_id'])
                        ->where('id_warehouse', $retail->warehouse_id)
                        ->first();
                    // $ppn = (ValueAddedTaxModel::first()->ppn / 100) * $getPrice->harga_jual;
                    $price = $getPrice->harga_jual;
                ...
                $del = DirectSalesDetailModel::where('direct_id', $id)->whereNotIn('product_id', $products_arr)->delete();
                ...
                $hargaDiskon = $cost * $diskon;
                $hargaAfterDiskon = $cost - $hargaDiskon - $product['discount_rp'];
            ...
            foreach ($jurnal_detail as $detail) {
                if ($detail->debit != 0) {
                    ...
                    // else {
                    //     $detail->credit = $selected_direct->total_ppn;
                    // }
                }
                $detail->save();
            }
    ...
    public function selectReturn()
    {
        try {
            ...
            if (request()->has('q')) {
                ...
                $product = DirectSalesDetailModel::join('products', 'products.id', '=', 'direct_sales_details.product_id')->join('product_sub_materials', 'product_sub_materials.id', '=', 'products.id_sub_material')->join('product_sub_types', 'product_sub_types.id', '=', 'products.id_sub_type')->select('products.nama_barang AS nama_barang', 'products.id AS id', 'product_sub_types.type_name AS type_name', 'product_sub_materials.nama_sub_material AS nama_sub_material')->where('direct_id', $retail_id)->get();
            }
    ...
    public function getQtyDetail()
    {
        ...
        if ($selected_return != null) {
            foreach ($selected_return as $value) {
                $selected_detail = ReturnRetailDetailModel::where('return_id', $value->id)
        ...
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->editColumn('total_after_ppn', function ($data) {
                    ...
                    foreach ($data as $value) {
                        if (ReturnRetailModel::where('retail_id', $value->id)->sum('total') != null) {
                            $total_return += ReturnRetailModel::where('retail_id', $value->id)
                                ->where('isreceived', 1)
                                ->where('isapproved', 1)
                                ->sum('total');
                        }
                ...
                ->addColumn('action', function ($invoice) {
                    ...
                    return view('direct_sales._option_paid_management', compact('invoice', 'total_sale', 'total_return', 'total_credit', 'id_cust', 'name_cust', 'code_cust'))->render();
                })
    ...
    public function updatePaid(Request $request, $id)
    {
        ...
        try {
            ...
            //Save Journal
            $created_journal = createJournal($last_date, 'Pembayaran Direct No.' . $selected_retail->order_number, $selected_retail->warehouse_id);
            ...
            if ($created_journal != '' && $created_journal != null && $created_journal != false) {
                createJournalDetail($created_journal, $request->acc_coa, $selected_retail->order_number, $total_current_amount, 0);
                // createJournalDetail(
                //     $created_journal,
                //     '2-300',
                ...
                //     0
                // );
                createJournalDetail($created_journal, '1-200', $selected_retail->order_number, 0, $total_current_amount);
            }
        ...
        try {
            ...
            if ($total_return > 0) {
                ...
                if (round($direct->total_incl) - $total_credit == 0) {
                    $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Retur Penjualan Direct Tunai.' . $direct->order_number, $direct->warehouse_id);
                    ...
                    // ** Perubahan Saldo PPN Keluaran ** //
                    $get_coa_persediaan = Coa::where('coa_code', '2-300')->first()->id;
                    changeSaldoKurang($get_coa_persediaan, $direct->warehouse_id, $total_credit);
                    ...
                    if ($journal != '' && $journal != null && $journal != false) {
                        ...
                        createJournalDetail($journal, '4-102', $direct->order_number, $total_credit, 0);
                        // ** COA PPn Keluaran ** //
                        // createJournalDetail(
                        //     $journal,
                        //     '2-300',
                        ...
                        // );
                        // ** COA KAS ** //
                        createJournalDetail($journal, $data_kas, $direct->order_number, 0, $total_credit);
                    }
                } elseif ($direct_credit->count() > 0) {
                    ...
                    $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Retur Penjualan Direct Kredit.' . $direct->order_number, $direct->warehouse_id);

                    // ** Jika Jurnal Berhasil Disimpan ** //
                    if ($journal != '' && $journal != null && $journal != false) {
                        // ** COA Return Penjualan ** //
                        createJournalDetail($journal, '4-102', $direct->order_number, round($direct->total_incl) - $total_credit, 0);
                        // ** COA PPn Keluaran ** //
                        // createJournalDetail(
                        //     $journal,
                        //     '2-300',
                        ...
                        // );
                        // ** COA Piutang ** //
                        createJournalDetail($journal, '1-200', $direct->order_number, 0, round($direct->total_incl) - $total_credit);
                    }

                    // ** Perubahan Saldo Retur Penjualan ** //
                    $get_coa_p_masukan = Coa::where('coa_code', '4-102')->first()->id;
                    changeSaldoTambah($get_coa_p_masukan, $direct->warehouse_id, $total_credit + (round($direct->total_incl) - $total_credit));
    ...
    public function deleteInvoice($id)
    {
        try {
            ...
            return redirect()
                ->back()
                ->with('success', 'Direct Invoice ' . $sales_order->order_number . ' has been deleted');
        } catch (\Exception $e) {
            DB::rollback();
            // dd($e->getMessage());
            return redirect()
