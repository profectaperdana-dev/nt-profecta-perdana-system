...
class ReturnController extends Controller
{
        ...
        try {
            ...
            foreach ($return_detail as $value) {
                $stock = SecondProductModel::where('warehouses_id', $return->tradeInBy->warehouse_id)
                    ->where('products_id', $value->product_id)
                    ->first();
    ...
    public function delete_return_purchase($id)
    {
        try {
            ...
            // dd($return_detail);
            foreach ($return_detail as $value) {
                $stock = StockModel::where('products_id', $value->product_id)
                    ->where('warehouses_id', $return->purchaseOrderBy->warehouse_id)
                    ->first();
        ...
        try {
            ...
            foreach ($return_detail as $value) {
                if ($return->isapproved == 1) {
                    $stock = StockModel::where('products_id', $value->product_id)
                        ->where('warehouses_id', $return->salesOrderBy->warehouse_id)
                        ->first();
                    $stock->stock = $stock->stock - $value->qty;
                    $stock->save();
                }
    ...
    public function delete_return_direct($id)
    {
        try {
            ...
            foreach ($return_detail as $value) {
                if ($return->isapproved == 1) {
                    $stock = StockModel::where('products_id', $value->product_id)
                        ->where('warehouses_id', $return->retailBy->warehouse_id)
                        ->first();
                    $stock->stock = $stock->stock - $value->qty;
                    $stock->save();
    ...
    public function indexTradeInSale(Request $request)
    {
        if ($request->ajax()) {
            if (!empty($request->from_date)) {
                ...
                $return = ReturnTradeSaleModel::with('returnDetailsBy', 'created_by', 'secondSaleBy')->latest()->get();
            }
            return datatables()
                ->of($return)
                ->editColumn('total', function ($data) {
    ...
    public function indexTradeIn(Request $request)
    {
        ...
        if ($request->ajax()) {
            if (!empty($request->from_date)) {
                ...
                $return = ReturnTradePurchaseModel::with('returnDetailsBy', 'created_by', 'TradeInBy')->where('return_date', date('Y-m-d'))->latest()->get();
            }
            return datatables()
                ->of($return)
                ->editColumn('total', function ($data) {
    ...
    public function index_purchase(Request $request)
    {
        ...
        if ($request->ajax()) {
            if (!empty($request->from_date)) {
                ...
                $return = ReturnPurchaseModel::with('returnDetailsBy', 'createdBy', 'purchaseOrderBy')->where('return_date', date('Y-m-d'))->latest()->get();
            }
            return datatables()
                ->of($return)
                ->editColumn('total', function ($data) {
    ...
    public function index_retail(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('action', function ($return) {
                    // $ppn = ValueAddedTaxModel::first()->ppn / 100;
                    return view('returns._option_retail', compact('return'))->render();
                })
                ->rawColumns(['return_reason'], ['action'])
                // ->rawColumns()
                ->addIndexColumn()
        ...
        foreach ($selected_so->salesOrderDetailsBy as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnDetailModel::where('return_id', $detail->id)
    ...
    public function createTradeInSale(Request $request, $id)
    {
        ...
        foreach ($selected_so->second_sale_details as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnTradeSaleDetailModel::where('return_id', $detail->id)
                        ->where('product_id', $value->product_second_id)
                        ->first();
                    if ($selected_detail == null) {
                        $return += 0;
                    } else {
    ...
    public function createTradeIn(Request $request, $id)
    {
        ...
        foreach ($selected_so->tradeInDetailBy as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnTradePurchaseDetailModel::where('return_id', $detail->id)
                        ->where('product_id', $value->product_trade_in)
                        ->first();
                    if ($selected_detail == null) {
                        $return += 0;
                    } else {
        ...
        try {
            ...
            foreach ($request->returnFields as $item) {
                ...
                $selected_pod = SecondSaleDetailModel::where('second_sale_id', $model->second_sale_id) //TradeInDetailModel
                    ->where('product_second_id', $detail->product_id)
                    ->first();
    ...
    public function create_item_promotion(Request $request, $id)
    {
        ...
        foreach ($selected_ip->transactionDetailBy as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnItemPromotionDetailModel::where('return_id', $detail->id)
    ...
    public function create_purchase_item_promotion(Request $request, $id)
    {
        ...
        foreach ($selected_ip->purchaseDetailBy as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnItemPromotionPurchaseDetailModel::where('return_id', $detail->id)
        ...
        try {
            ...
            foreach ($request->returnFields as $item) {
                ...
                $selected_pod = TradeInDetailModel::where('trade_in_id', $model->trade_in_id)
                    ->where('product_trade_in', $detail->product_id)
                    ->first();
    ...
    public function create_purchase(Request $request, $id)
    {
        ...
        foreach ($selected_po->purchaseOrderDetailsBy as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnPurchaseDetailModel::where('return_id', $detail->id)
    ...
    public function create_retail(Request $request, $id)
    {
        ...
        foreach ($selected_retail->directSalesDetailBy as $value) {
            ...
            if ($selected_return != null) {
                foreach ($selected_return as $detail) {
                    $selected_detail = ReturnRetailDetailModel::where('return_id', $detail->id)
                        ->where('product_id', $value->product_id)
                        ->first();
                    if ($selected_detail == null) {
                        $return += 0;
    ...
    public function receiving_indirect(Request $request)
    {
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('action', function ($return) {
                    // $ppn = ValueAddedTaxModel::first()->ppn / 100;
                    return view('returns._option_receiving_indirect', compact('return'))->render();
                })
                ->rawColumns(['return_reason'], ['action'])
                // ->rawColumns()
                ->addIndexColumn()
    ...
    public function approval_indirect(Request $request)
    {
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('action', function ($return) {
                    // $ppn = ValueAddedTaxModel::first()->ppn / 100;
                    return view('returns._option_approval_indirect', compact('return'))->render();
                })
                ->rawColumns(['return_reason'], ['action'])
                // ->rawColumns()
                ->addIndexColumn()
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $length = 3;
            $last_record = ReturnModel::where('return_number', 'like', "%$similar_number%")
                ->latest()
                ->first();
            if ($last_record) {
            ...
            foreach ($request->returnFields as $item) {
                ...
                $selected_sod = SalesOrderDetailModel::where('sales_orders_id', $model->sales_order_id)
                    ->where('products_id', $detail->product_id)
                    ->first();
            ...
            DB::rollback();
            dd($e);
            return redirect('/return')->with('error2', $e->getMessage() . '. Please call your Most Valuable IT Team.');
        }
    }
    ...
    public function receive_indirect(Request $request, $id)
    {
        ...
        try {
            ...
            $del = ReturnDetailModel::where('return_id', $id)->whereNotIn('product_id', $products_arr)->delete();
    ...
    public function approve_indirect(Request $request, $id)
    {
        ...
        try {
            ...
            if ($selected_so->isPaid == 0) {
                $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Retur Penjualan Indirect Kredit.' . $selected_so->order_number, $selected_so->warehouse_id);

                // ** Jika Jurnal Berhasil Disimpan ** //
                if ($journal != '' && $journal != null && $journal != false) {
                    // ** COA Return Penjualan ** //
                    createJournalDetail($journal, '4-102', $selected_return->return_number, round($selected_so->total_after_ppn) / 1.11, 0);
                    // ** COA PPn Keluaran ** //
                    createJournalDetail($journal, '2-300', $selected_return->return_number, round($selected_so->total_after_ppn) / 1.11, 0);
                    // ** COA Piutang ** //
                    createJournalDetail($journal, '1-200', $selected_return->return_number, 0, round($selected_so->total_after_ppn));
                }
            }
            ...
            $get_hpp_detail = JournalDetail::where('journal_id', $selected_so->id_jurnal_hpp)
                ->where('debit', '>', 0)
                ->first();

            $hpp = createJournal(Carbon::now()->format('Y-m-d'), 'Persediaan Bertambah.' . $selected_return->return_number, $selected_so->warehouse_id);
            ...
            if ($hpp != '' && $hpp != null && $hpp != false) {
                ...
                createJournalDetail($hpp, '1-401', $selected_return->return_number, $get_hpp_detail->debit, 0);

                //HPP
                createJournalDetail($hpp, '6-000', $selected_return->return_number, 0, $get_hpp_detail->debit);
            }
    ...
    public function store_purchase(Request $request)
    {
        ...
        try {
            ...
            if ($data_kredit->count() > 0) {
                ...
                $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Retur Persediaan Pembelian Kredit.' . $selected_po->order_number, $selected_po->warehouse_id);

                // ** Jika Jurnal Berhasil Disimpan ** //
                if ($journal != '' && $journal != null && $journal != false) {
                    // ** COA Hutang Dagang ** //
                    createJournalDetail($journal, '2-101', $selected_po->order_number, round($selected_po->total), 0);
                    // ** COA Return Pembelian ** //
                    createJournalDetail($journal, '5-103', $selected_po->order_number, 0, round($selected_po->total) / 1.11);
                    // ** COA PPn Masukan ** //
                    createJournalDetail($journal, '1-600', $selected_po->order_number, 0, (round($selected_po->total) / 1.11) * (ValueAddedTaxModel::first()->ppn / 100));
                }
    ...
    public function receiving_retail(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('action', function ($return) {
                    // $ppn = ValueAddedTaxModel::first()->ppn / 100;
                    return view('returns._option_receiving_retail', compact('return'))->render();
                })
                ->rawColumns(['return_reason'], ['action'])
                // ->rawColumns()
                ->addIndexColumn()
    ...
    public function approval_retail(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return datatables()
                ...
                ->addColumn('action', function ($return) {
                    // $ppn = ValueAddedTaxModel::first()->ppn / 100;
                    return view('returns._option_approval_retail', compact('return'))->render();
                })
                ->rawColumns(['return_reason'], ['action'])
                // ->rawColumns()
                ->addIndexColumn()
    ...
    public function store_retail(Request $request)
    {
        try {
            ...
            $length = 3;
            $last_record = ReturnRetailModel::where('return_number', 'like', "%$similar_number%")
                ->latest()
                ->first();
            if ($last_record) {
                $last_three_digits = (int) substr($last_record->return_number, -3);
            } else {
                $last_three_digits = 0; // atau nilai default lainnya sesuai kebutuhan
            }
            $next_three_digits = $last_three_digits + 1;
            ...
            foreach ($request->returnFields as $item) {
                ...
                if ($detail->price == null) {
                    $product = ProductCostModel::where('id_product', $detail->product_id)
        ...
        try {
            ...
            foreach ($request->returnFields as $product) {
                ...
                if ($price == 0 || $price == null) {
                    $products = ProductCostModel::where('id_product', $product['product_id'])
                        ->where('id_warehouse', $selected_retail->warehouse_id)
                        ->first();
            ...
            $saved = $selected_return->save();
            // dd($products_arr);
            $del = ReturnRetailDetailModel::where('return_id', $id)->whereNotIn('product_id', $products_arr)->delete();
    ...
    public function approve_retail(Request $request, $id)
    {
        ...
        try {
            ...
            if ($selected_retail->isPaid == 0) {
                $journal = createJournal(Carbon::now()->format('Y-m-d'), 'Retur Penjualan Direct Kredit.' . $selected_retail->order_number, $selected_retail->warehouse_id);

                // ** Jika Jurnal Berhasil Disimpan ** //
                if ($journal != '' && $journal != null && $journal != false) {
                    // ** COA Return Penjualan ** //
                    createJournalDetail($journal, '4-102', $selected_return->return_number, round($selected_retail->total_incl) / 1.11, 0);
                    // ** COA PPn Keluaran ** //
                    createJournalDetail($journal, '2-300', $selected_return->return_number, round($selected_retail->total_incl) / 1.11, 0);
                    // ** COA Piutang ** //
                    createJournalDetail($journal, '1-200', $selected_return->return_number, 0, round($selected_retail->total_incl));
                }
            }
            ...
            $get_hpp_detail = JournalDetail::where('journal_id', $selected_retail->hpp_id)
                ->where('debit', '>', 0)
                ->first();

            $hpp = createJournal(Carbon::now()->format('Y-m-d'), 'Persediaan Bertambah.' . $selected_return->return_number, $selected_retail->warehouse_id);
        ...
        try {
            ...
            foreach ($returnDetail as $value) {
                $getCostModel = ItemPromotionCostModel::where('item_id', $value->id_item)
                    ->where('warehouse_id', $selected_so->id_warehouse)
    ...
    public function store_purchase_item_promotion(Request $request)
    {
        ...
        try {
            ...
            foreach ($returnDetail as $value) {
                $getCostModel = ItemPromotionCostModel::where('item_id', $value->item_id)
                    ->where('warehouse_id', $selected_so->warehouse_id)
                    ->where('cost', $value->price)
                    ->first();
                if ($getCostModel != null) {
                    $getCostModel->qty = $getCostModel->qty - $value->qty;
    ...
    public function update_return(Request $request, $id)
    {
        ...
        try {
            ...
            $warehouse = WarehouseModel::where('id_area', $customer->area_cust_id)
                ->where('type', 5)
                ->first();
            foreach ($return_restore as $restore) {
                $stock = StockModel::where('warehouses_id', $warehouse->id)
                    ->where('products_id', $restore->product_id)
                    ->first();
            ...
            if ($journal) {
                ...
                $akun_hutang = JurnalDetailModel::where('expenses_id', $journal->id)
                    ->where('account_id', '600.05')
                    ->first();
                ...
                // akun pajak keluaran
                $akun_pajak = JurnalDetailModel::where('expenses_id', $journal->id)
                    ->where('account_id', '600.07')
                    ->first();
                ...
                $akun_pembelian = JurnalDetailModel::where('expenses_id', $journal->id)
                    ->where('account_id', '100.01.04')
                    ->first();
        ...
        try {
            ...
            foreach ($request->returnFields as $product) {
                $selected_rod = TradeInDetailModel::where('trade_in_id', $selected_return->trade_in_id)
                    ->where('product_trade_in', $product['product_id'])
                    ->first();
            ...
            foreach ($request->returnFields as $restore) {
                $second_stock = SecondProductModel::where('warehouses_id', $selected_retail->warehouse_id)
                    ->where('products_id', $restore['product_id'])
                    ->first();
        ...
        try {
            ...
            foreach ($request->returnFields as $product) {
                $selected_rod = SecondSaleDetailModel::where('second_sale_id', $selected_return->second_sale_id)
                    ->where('product_second_id', $product['product_id'])
                    ->first();
    ...
    public function update_return_purchase(Request $request, $id)
    {
        try {
            ...
            $del = ReturnPurchaseDetailModel::where('return_id', $id)->whereNotIn('product_id', $products_arr)->delete();
    ...
    public function print_return($id)
    {
        ...
        $pdf = FacadePdf::loadView('returns.print_return', compact('warehouse', 'data', 'ppn'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->return_number . '.pdf');
    ...
    public function print_return_purchase($id)
    {
        ...
        $pdf = FacadePdf::loadView('returns.print_return_purchase', compact('warehouse', 'data'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->return_number . '.pdf');
    ...
    public function printTradeIn($id)
    {
        ...
        $pdf = FacadePdf::loadView('returns.print_return_trade_purchase', compact('warehouse', 'data', 'ppn'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->return_number . '.pdf');
    ...
    public function printTradeInSale($id)
    {
        ...
        $pdf = FacadePdf::loadView('returns.print_return_trade_sale', compact('warehouse', 'data', 'ppn'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->return_number . '.pdf');
    ...
    public function print_item_promotion($id)
    {
        ...
        $data->pdf_return = $data->return_number . '.pdf';
        $data->save();
        $pdf = FacadePdf::loadView('returns.print_item_promotion', compact('warehouse', 'data'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->return_number . '.pdf');
        return $pdf->stream($data->pdf_return);
    }
    ...
    public function print_return_retail($id)
    {
        ...
        $pdf = FacadePdf::loadView('returns.print_return_retail', compact('warehouse', 'data'))
            ->setPaper('A5', 'landscape')
            ->save('pdf/' . $data->return_number . '.pdf');
